...
def is_valid_row(row, mmsi_column, lat_column, lon_column):
    """Filters out 0 coordinates and those more than 1000km away from Copenhagen"""
...
def merge_and_sort_shard(args):
    """External Merge Sort for Shards"""
...
if __name__ == "__main__":
    ...
    print(f"Filtering and Sharding ({num_cores} processes)")
    ...
    print(f"Processed {total_processed:,} rows. Kept {total_kept:,} rows. Took {execution_time1:.2f} seconds.")

    print(f"\nMerging and Sorting ({num_cores} processes)")
    ...
    print(f"{num_cores} final partitioned files generated in {OUTPUT_DIR}. Took {execution_time2:.2f} seconds.")
